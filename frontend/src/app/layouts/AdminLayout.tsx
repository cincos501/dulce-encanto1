...
export default function AdminLayout() {
  ...
  return (
    <div className="min-h-screen bg-background text-text-main flex font-sans overflow-x-hidden relative">
      ...
      <aside 
        className={`bg-surface border-r border-border transition-all duration-300 hidden lg:flex flex-col justify-between fixed h-full z-20 ${
                ...
                {filteredMenuItems.filter(i => i.section === 'none').map((item) => {
                  ...
                  return (
                    <Link
                      key={item.name}
                      to={item.path}
                      className={`flex items-center gap-3 px-4 py-2.5 rounded-lg text-xs font-semibold transition-all duration-205 ${
                        isActive
                          ? 'bg-secondary/15 text-primary border border-secondary/35 font-bold'
      ...
      <div 
        className={`flex-grow min-h-screen flex flex-col transition-all duration-300 w-full ${
